...
describe("GET /api/v1/admin/users", () => {
  ...
  it("refreshes session and forwards list query to the backend", async () => {
    ...
    const request = new NextRequest("http://localhost/api/v1/admin/users?page=0&size=20", {
      method: "GET",
      headers: {
    ...
    expect(cookies).toContain("rentflow_role=CUSTOMER%2CADMIN");
  });
  ...
  it("forwards backend authorization failures", async () => {
    ...
    const request = new NextRequest("http://localhost/api/v1/admin/users?page=0&size=1", {
      method: "GET",
    });
