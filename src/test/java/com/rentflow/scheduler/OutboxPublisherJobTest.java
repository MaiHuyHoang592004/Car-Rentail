...
class OutboxPublisherJobTest {
    ...
    void runInvokesPublisherWhenEnabled() {
        ...
        OutboxPublisherJob job = new OutboxPublisherJob(publisherService, true, 50, 5, 60);

        job.run();

        verify(publisherService).processBatch(50, 5, 60);
    }

    @Test
    void runSkipsWhenDisabled() {
        OutboxPublisherService publisherService = mock(OutboxPublisherService.class);
        OutboxPublisherJob job = new OutboxPublisherJob(publisherService, false, 50, 5, 60);
